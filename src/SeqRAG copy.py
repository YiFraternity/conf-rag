...
class SeqConfidenceRAG(BasicRAG):
    ...
    def modifier(self, question, ptext, text, docs):
        ...
        for i, sent in enumerate(sentences):
            current_history = history_resp + (' ' + sentences[i-1]) if i > 0 else history_resp
            conf_prompt = get_conf_prompt(
                question=question,
        ...
        # Batch generate confidence scores
        ...
        for i, (sent, conf_result) in enumerate(zip(sentences, conf_results)):
            ...
            try:
                data = clean_json_txt(conf_result)
                if isinstance(data, dict):
                    score = data.get("Score", 0.0) if self.confs_class == 'value' else data.get("Confidence", 0.0)
                    if isinstance(score, str):
                        score = float(score)
                    if self.confs_class == 'level':
                        score = score / 5.0
                    if not (0 <= score <= 1):
                        score = 0.0
                else:
                    score = 0.0
                conf_type = 'value' if self.confs_class == 'value' else 'level'
            except:
                score = 0.0
                conf_type = 'value'
            ...
            if "use_reflect" not in self.__dict__ or not self.use_reflect:
                if score >= 0.5:  # Assuming high confidence
                    confs_value = 1
                else:
                    confs_value = -1
            else:
            ...
            if confs_value == 0 and reflect_tag:
                print(f'cur confs:{confs_value}, performed reflect')
